...
class CalcMesh:
    ...
    def __init__(self, nodes_coords, tetrs_points):
        ...
        self.velocity = np.ndarray((3, int(len(nodes_coords) / 3)), dtype = np.double)
        for k in range(3):
            for i in range(int(len(nodes_coords) / 3)):
                if (k==2):
                    self.velocity[k][i] = self.nodes[k][i]/20 - (self.nodes[k][i])**2/20
                else:
                    x0 = self.nodes[0][i]
                    y0 = self.nodes[1][i]
                    A = - x0/y0
                    ...
                    y1 = A * x1
                    if (k==0):
                        self.velocity[k][i] = x1

                    else:
                        self.velocity[k][i] = y1





            #np.ones(shape=(3, int(len(nodes_coords) / 3)), dtype=np.double)
    ...
    def vel_renew(self):
        for k in range(3):
            for i in range(int(len(self.nodes) / 3)):
                if (k==2):
                    self.velocity[k][i] = self.nodes[k][i]/20 - (self.nodes[k][i])**2/20
                else:
                    x0 = self.nodes[0][i]
...
try:
    ...
    gmsh.merge(os.path.join(path, 'Comic.stl'))
except:
